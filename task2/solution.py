...
class Model(object):
    ...
    def __init__(self):
        ...
        if use_densenet:
            ...
            # Use Bayesian Neural Network directly from torchbnn library
            # Code from https://github.com/Harry24k/bayesian-neural-network-pytorch
            # self.network = nn.Sequential(
            #     bnn.BayesLinear(prior_mu=0, prior_sigma=0.1, in_features=28 * 28, out_features=100),
            #     nn.ReLU(),
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    # loss = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                    ce = self.ce_loss(current_logits, batch_y)
                    kl = self.kl_loss(self.network)
    ...
    def predict(self, data_loader: torch.utils.data.DataLoader) -> np.ndarray:
        ...
        for batch_x, batch_y in data_loader:
            ...
            current_probabilities = self.network.predict_probabilities(batch_x).detach()

            # probability_samples = torch.stack([F.softmax(self.network.forward(batch_x), dim=1) for _ in range(10)], dim=0)
            # current_probabilities = torch.mean(probability_samples, dim=0).detach()

            current_probabilities = current_probabilities.to('cpu').numpy()
...
class BayesianLayer(nn.Module):
    ...
    """
    def __init__(self, prior_mu: float, prior_sigma: float, in_features: int, out_features: int, bias: bool = True):
        """
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: typing.Tuple[int, ...], out_features: int):
        ...
        self.layers = nn.ModuleList([
            BayesianLayer(0, 0.1, feature_sizes[idx], feature_sizes[idx + 1], bias=True)
