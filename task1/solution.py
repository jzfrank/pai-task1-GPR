...
class NaiveGP():
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        N = 1000
        print(f"------fitting model (NaiveGP) with N = {N}-------")
        start = time.time()
        ...
        if train_x.shape[0] > N:
            train_x_ = train_x[:N, :]
            train_y_ = train_y[:N]
            self.gpr.fit(train_x_, train_y_)
        else:
        ...
        print(f"------time used to fit: {time.time() - start}--------")
...
class Model(object):
    ...
    def __init__(self, num_model=1):
        ...
        # self.model = NaiveGP()
        print("------INIT MODEL-------")
        self.models = [NaiveGP() for _ in range(num_model)]
    ...
    def predict(self, x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        predictions, gp_mean, gp_std = self.models[0].predict(x)
        # for model in self.models[1:]:
        #     p, m, s = model.predict(x)
        #     predictions += p
        #     gp_mean += m
        #     gp_std += s
        # print(f"len(self.models): {len(self.models)}")
        # predictions *= 1/len(self.models)
        # gp_mean *= 1/len(self.models)
        # gp_std *= 1/len(self.models) ** 2
        print(f"difference between averaged prediction and single prediction: "
              f"{np.mean(abs(predictions - self.models[0].predict(x)))}")
        return predictions, gp_mean, gp_std
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        """
        [i.fit_model(train_x, train_y) for i in self.models]
